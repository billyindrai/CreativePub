...
@foreach($gallery as $g)
    <form method="POST" action="/edit_gallery_no_cover" id="galleryForm" enctype="multipart/form-data">
    @csrf
    <div class="container-2xl md:flex grid grid-cols-2 px-32">
    <input type="hidden" id="token" value="{{ @csrf_token() }}">
    <input type="hidden" name="galleryId" id="galleryId" value="{{ $g->idGallery }}">
            ...
            <div class="container-2xl md:flex flex-1 justify-center px-44">
                <div class="container">
                    <div class="container">
                        ...
                        <input name="titleGallery" id="titleGallery" class=" bg-white rounded-lg w-10/12 h-10 mt-2" type="text" value="{{$g->titleGallery}}" required>
                    </div>
                    ...
                    <div class="container-2xl mt-4">
                        ...
                        <textarea id="descriptionGallery" name="descriptionGallery" class="bg-white rounded-lg w-10/12 h-32 mt-2" form="galleryForm"  required>{{$g->descriptionGallery}}</textarea>
                    </div>
                    <div class="container-2xl mt-4">
                        <p class="text-white text-xl font-semibold ">Project Tags</p>
                        <input id="tagsGallery" name="tagsGallery" class=" bg-white rounded-lg w-10/12 h-16 mt-2" value="{{$g->tagsGallery}}" type="text">
                    </div>
                </div>
            </div>
        ...
        $("#galleryForm button").click(function(e){
            ...
            if ($(this).attr("value") == "continue") {            
                ...
                if($('#fileGallery').val()){
                    ...
                    formData.append('galleryId', $('#galleryId').val());
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    var htmlCode = "";
                        ...
                        htmlCode  = '<form method="POST" action="/edit_gallery_cover" id="galleryCoverForm" enctype="multipart/form-data">' +
                            '@csrf' +
                            '<input type="text" name="galleryId" id="galleryId" value="'+response+'"class="hidden">' +
                            '<div class="container-2xl py-16 px-36 items-center bg-zinc-800 rounded-lg">' +
                                    '<div class="container">'+
                                        '<p class="text-white text-center font-medium px-8 text-xl mb-9">Upload your Collection Cover</p>' +
                                        '<div class="container md:flex justify-center">'+

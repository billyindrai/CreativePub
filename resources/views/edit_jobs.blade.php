...
<form method="POST" action="/store_edited_job" id="jobForm">
    @csrf
    <div class="container-2xl md:flex justify-center">
        <div class="container w-fit">
        @foreach($jobDetail as $jd)
            <input type="hidden" id="idJob" name="idJob" value="{{$jd->idJob}}">
            <div class="container">
                <p class="text-white text-xl font-semibold ">Job Tittle</p>
                <input name="titleJob" id="titleJob"  class=" bg-white rounded-lg w-80 h-10 mt-2" value="{{$jd->titleJob}}" type="text" required>
            </div>
            <div class="container mt-4">
                <p class="text-white text-xl font-semibold ">Tools Used</p>
                <select form="jobForm"  class="bg-white rounded-lg w-80 h-10 mt-2" name="toolsJob" id="toolsJob" form="jobForm" value="{{$jd->toolsJob}}" required>
            ...
            <div class="container mt-4">
                ...
                <select class="bg-white rounded-lg w-80 h-10 mt-2" name="categoryJob" id="categoryJob" form="jobForm" value="{{$jd->categoryJob}}" required>
            ...
            <div class="container mt-4">
                ...
                <textarea id="descriptionJob" name="descriptionJob" class="bg-white rounded-lg w-80 h-32 mt-2" form="jobForm" required>{{$jd->descriptionJob}}</textarea>
            </div>
            <div class="container mt-4">
                <p class="text-white text-xl font-semibold ">Job Tags</p>
                <input name="tagsJob" id="tagsJob" class=" bg-white rounded-lg w-80 h-16 mt-2" value="{{$jd->tagsJob}}" type="text" required>
            </div>
            <div class="container mt-4">
                ...
                <div class="relative">
                    ...
                    <input id="dueDateJob" name="dueDateJob"  datepicker datepicker-title="Due Date Job" datepicker-buttons type="text" class="rounded-lg block w-full pl-10 p-2.5" value="{{$jd->dueDateJob}}" placeholder="Select date" required>
                </div>
            </div>
        @endforeach
        </div>
    ...
    <div class="md:flex justify-center mt-16 ">
        <div>
            <button id="button" name="button" value="publish"  class="bg-register_orange rounded-lg w-80 h-10 text-white text-base font-medium" >Update</button>
        </div>
    </div>
    <div class="md:flex justify-center mt-6">
        <div>
            <button id="button" name="button" value="cancel"  class="bg-white rounded-lg w-80 h-10 text-register_orange text-base font-medium">Cancel</button>
        </div>
    </div>
</form>
